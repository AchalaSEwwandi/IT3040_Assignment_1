...
def _find_chat_locators(page, timeout_ms: int):
    ...
    while time.time() < deadline:
        ...
        try:
            # Try Chat Sinhala placeholders
            input_by_ph = page.locator('textarea[placeholder*="Singlish"]').first
            output_by_ph = page.locator('textarea[placeholder*="Transliterated"]').first
            ...
            # Try English/Sinhala placeholders
...
def run_test():
    ...
    with sync_playwright() as p:
        ...
        if is_chat:
            # Auto-select Chat Sinhala mode
            try:
                page.get_by_text("Transliteration").first.click(timeout=5000)
                page.wait_for_timeout(1000)
                page.get_by_text("Chat Sinhala").first.click(timeout=5000)
                page.wait_for_timeout(3000)
                print("Chat Sinhala mode selected.")
            except Exception as e:
                print(f"Warning: Could not auto-select Chat Sinhala: {e}")
            try:
